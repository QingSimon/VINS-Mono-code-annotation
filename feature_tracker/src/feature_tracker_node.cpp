...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // 发布频率控制
    // 并不是每读入一帧图像，就要发布特征点
    // 通过判断间隔时间内的发布次数来控制发布频率
    // 如果需要发布，则将PUB_THIS_FRAME置为true，否则置为FALSE
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)//不等号左边实际上在计算统计时间间隔内数据的平均发布频率
    {
        // 当平均发布频率小于设定频率时，发布当前帧的数据
        PUB_THIS_FRAME = true;

        // 当统计时间间隔内的数据平均发布频率十分接近设定频率时，更新时间间隔起始时刻，并将数据发布次数置0
        // 不知道这么做有什么意义，难道是防止pub_count溢出了？
        if (abs(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time) - FREQ) < 0.01 * FREQ)
        {
            first_image_time = img_msg->header.stamp.toSec();//将当前帧时刻设置为发布第一帧数据的时刻
